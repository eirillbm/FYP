...
[Pos,Vel,Acc]=read_virtual(h,N);
save('SIMA_data.mat','Pos','Vel','Acc');
%load('SIMA_data.mat');
...
% figure
% plot(t,vel(1,:)-Vel(1,:),'k');
% hold on 
% plot(t,vel(2,:)-Vel(2,:),'k--');
% plot(t,vel(3,:)-Vel(3,:),'k:');
% plot(t,vel(4,:)-Vel(4,:),'r');
% plot(t,vel(5,:)-Vel(5,:),'r--');
% plot(t,vel(6,:)-Vel(6,:),'r:');
% hold off
% legend('u','v','w','p','q','r')
...
v1=zeros(12,N);
for j=1:6
    uv(j,:) = awgn(acc(j,:),100,'measured');       %SNR=100dB (0.00000001% noise)
    uv2(j,:) = awgn(acc(j,:),100,'measured');      %SNR=100dB (0.00000001% noise)
end
%SNR=20dB (1% noise), SNR=16.99dB (2% noise), SNR=13.01dB (5% noise), SNR=100dB (10% noise), SNR=8.24dB (15% noise)
v1(1:6,:) = (uv - acc);
v1(7:12,:) = (uv2 - acc);
...
yv=zeros(12,N);
 for j=1:12
     yv(j,:) = awgn(y(j,:),100,'measured');       %SNR=100dB (0.00000001% noise)
 end
%SNR=20dB (1% noise), SNR=13.01dB (5% noise), SNR=10dB (10% noise), SNR=8.24dB (15% noise)
v2 = (yv - y);
...
% Compute measurement error covariance
MeasErr = zeros(12,N);
