...
[Pos,V,Acc,V_est]=read_physical(N);
...
% Process noise vectors (white, Gaussian, zero-mean)
 ...
 noiseval=[20 20.46 20.97 21.55 22.22 23.01 23.98 25.23 26.99 30 30.46 33.01 40 43.01 50 53.01 60];
 for m=1:17
 for j=1:3
     uv(j,:) = awgn(u(j,:),noiseval(m),'measured');        %SNR=100dB (0.00000001% noise)
     uv2(j,:) = awgn(u(j,:),100,'measured');
     uv3(j,:) = awgn(u(j,:),100,'measured');
     uv4(j,:) = awgn(u(j,:),20,'measured');        %SNR=20dB (1% noise)
 end
...
% Write to file
fileID = fopen('noiseval.txt','w');
fprintf(fileID,'%f ',noiseval);
fprintf(fileID,'\n');
